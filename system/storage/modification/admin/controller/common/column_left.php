...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
      	...
      	$menu_item_status = $this->config->get('theme_frame_status');
      
      	$frame_stores = array();
      	
      	$frame_stores[] = array(
					'name'     => $this->config->get('config_name'),
					'href'     => $this->url->link('extension/theme/frame', 'user_token=' . $this->session->data['user_token']. '&store_id=0', true),
					'children' => array()
				);
      	...
      	$stores = $this->model_setting_store->getStores();
				...
				foreach ($stores as $store) {
					...
					$store_info = $this->model_setting_setting->getSetting('theme_frame', $store['store_id']);

					if (isset($store_info['theme_frame_status']) && $store_info['theme_frame_status']) {
						$menu_item_status = $store_info['theme_frame_status'];
					}
				
					$frame_stores[] = array(
						'name'     => $store['name'],
						'href'     => $this->url->link('extension/theme/frame', 'user_token=' . $this->session->data['user_token'] . '&store_id=' . $store['store_id'], true),
						'children' => array()
					);
				}

        if ($this->user->hasPermission('access', 'extension/theme/frame') && $menu_item_status) {					
					$data['menus'][] = array(
						'id'       => 'menu-frame',
						'icon'	   => 'fa-sliders invisible', 
						'name'	   => 'Frame',
						'href'     => '',
						'children' => $frame_stores
					);		
				}
