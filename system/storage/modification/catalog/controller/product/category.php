...
class ControllerProductCategory extends Controller {
	public function index() {
        ...
        if ($this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_width')) {
          $category_img_size_w = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_width');
        } else {
          $category_img_size_w = 120;
        }

        if ($this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_height')) {
          $category_img_size_h = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_category_height');
        } else {
          $category_img_size_h = 120;
        }

			  $image1x = $this->model_tool_image->resize($result['image'], $category_img_size_w, $category_img_size_h);

        if ($hd_imgs) {
          $image2x = $this->model_tool_image->resize($result['image'], $category_img_size_w*2, $category_img_size_h*2);
          $image3x = $this->model_tool_image->resize($result['image'], $category_img_size_w*3, $category_img_size_h*3);
          $image4x = $this->model_tool_image->resize($result['image'], $category_img_size_w*4, $category_img_size_h*4);
        }
      
				$data['categories'][] = array(

        'img_width' => $category_img_size_w . 'px',
        'img_height' => $category_img_size_h . 'px',
				'image1x' => $image1x,
        'image2x' => $hd_imgs ? $image2x : NULL,
        'image3x' => $hd_imgs ? $image3x : NULL,
        'image4x' => $hd_imgs ? $image4x : NULL,
        ...
        $delimiter = '<!--more-->';
        $split_description = explode($delimiter, html_entity_decode($category_info['description'], ENT_QUOTES, 'UTF-8'), 2);

        if ($this->ocfilter && $this->ocfilter->getParams()) {
          $split_description = explode($delimiter, html_entity_decode($this->ocfilter->getPageDescription(), ENT_QUOTES, 'UTF-8'), 2);
        }

        if (count($split_description) > 1) {
          $data['splited_description_top'] = html_entity_decode($split_description[0], ENT_QUOTES, 'UTF-8');
          $data['splited_description_bottom'] = html_entity_decode($split_description[1], ENT_QUOTES, 'UTF-8');
        } else {
          $data['splited_description_top'] = false;
          $data['splited_description_bottom'] = false;
        }
